...
@section('content')
                    ...
                    <table class="table table-bordered table-striped" id="clientGroupTable">
                        <thead>
                            <tr>
                                ...
                                @if (Auth::user()->hasRolePermission('client-groups.update') || Auth::user()->hasRolePermission('client-groups.destroy'))
                                    <th>Actions</th>
                                @endif
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($clientGroups as $group)
                                <tr>
                                    ...
                                    @if (Auth::user()->hasRolePermission('client-groups.update') || Auth::user()->hasRolePermission('client-groups.destroy'))
                                        <td>
                                            @if (Auth::user()->hasRolePermission('client-groups.update'))
                                                <a href="#"
                                                    class="btn search"onclick="openEditModal({{ $group->id }}, '{{ $group->name }}', {{ $group->client_id }})">
                                                    <i class="fa-solid fa-pencil" title="Edit"></i>
                                                </a>
                                            @endif
                                            @if (Auth::user()->hasRolePermission('client-groups.destroy'))
                                                <form id="Model-Form"
                                                    action="{{ route('client-groups.destroy', $group->id) }}" method="POST"
    ...
    <!-- Modal -->
    <div class="modal fade modal-margin" id="clientGroupModal" tabindex="-1" aria-labelledby="clientGroupModalLabel"
        aria-hidden="true">
